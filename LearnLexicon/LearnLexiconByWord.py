...
parser.add_option("--steps", dest="steps", type="int", default=10, help="Number of samples to run")
parser.add_option("--top", dest="top_count", type="int", default=3, help="Top number of hypotheses to store")
...
parser.add_option("--dmax", dest="data_max", type="int", default=51, help="Max data to run")
...
#   Chain Function
######################################################################################################
def run(word, data_amount):
    print "Starting chain on %s data points for %s"%(data_amount, word)
    data = makeWordData(word, target, four_gen_tree_context, n=int(data_amount), alpha=options.alpha, epsilon=options.epsilon, verbose=True)

    h0 = KinshipLexicon(alpha=options.alpha)
    h0.set_word(word, LOTHypothesis(my_grammar, display='lambda recurse_, C, X: %s'))
